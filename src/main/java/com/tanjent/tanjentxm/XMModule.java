...
public class XMModule {
		...
		for (int c=0;c<this.numberOfChannels;c++) {
			...
			if (this.firstTick == true) {
				...
				if (this.channel.note.effectType == 0x00 && this.channel.note.effectParameter > 0) {
					...
					this.channel.samplePositionAddTickEnd = (XMModule.amigaPeriodToFrequency(this.channel.amigaPeriod + this.channel.autoVibratoAmigaPeriodAdd + this.channel.vibratoAmigaPeriodAdd) * (float)Math.pow(2, this.arpeggioNoteAdd / 12.0)) / this.sampleRate;
					this.channel.samplePositionAddTickStart = this.channel.samplePositionAddTickEnd;
				}
			...
			if (this.instruments[this.channel.instrumentColumnMemory].panningEnvelopeEnabled == true) {
				...
				float panningEnvelopeOffset = (this.channel.panningEnvelopeValue - 32) / 64.0f;
	...
	{	
		...
		if (this.channel.note.effectType == 0x0D)
		{
			...
			this.patternBreakRow = this.channel.note.effectParameter & 0xFF;
			return;
		}
	...
	{		
		...
		if (instrument.volumeEnvelopeEnabled == true)
		{
			...
			channel.volumeEnvelopeValueTickAdd = ((instrument.volumeEnvelopeValues[1] - instrument.volumeEnvelopeValues[0]) / XMChannel.MAX_VOLUME) / (float)instrument.volumeEnvelopeTicks[1];
		}
	...
	{	
		...
		{			
			if (instrument.volumeEnvelopeTicks[channel.volumeEnvelopePoint] == channel.volumeEnvelopeTick)
			{
				...
				channel.volumeEnvelopeValue = instrument.volumeEnvelopeValues[channel.volumeEnvelopePoint] / XMChannel.MAX_VOLUME;
				channel.volumeEnvelopeValueTickAdd = ((instrument.volumeEnvelopeValues[nextPoint] - instrument.volumeEnvelopeValues[channel.volumeEnvelopePoint]) / XMChannel.MAX_VOLUME) / (float)(instrument.volumeEnvelopeTicks[nextPoint] - instrument.volumeEnvelopeTicks[channel.volumeEnvelopePoint]);
	...
	public boolean loadXM(byte[] data)
	{
		...
		if (this.mixFactor < 0) {
			this.mixFactorFP = FixedPoint.FLOAT_TO_FP((float)Math.sqrt((1.0 / this.channels.length)));
		} else if (this.mixFactor == 0) {
			this.mixFactorFP = FixedPoint.FLOAT_TO_FP((float)(1.0 / this.channels.length)); 
		} else {
